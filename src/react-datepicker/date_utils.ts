...
export function parseDate(value, dateFormat, locale, strictParsing) {
  ...
  let localeObject: any = getLocaleObject(locale) || getDefaultLocale();
...
export function safeDateFormat(date, { dateFormat, locale }: { dateFormat, locale?}) {
  return (
    (date &&
      formatDate(
        date,
...
export function getWeek(date, locale) {
  ...
  return dfgetWeek(date, localeObj ? { locale: localeObj } : {});
}
...
export function getStartOfToday() {
  return startOfDay(newDate() || new Date());
}
...
export function setDefaultLocale(localeName) {
  ...
  scope['__localeData__'] = localeName;
}
...
export function isDayDisabled(
  day,
  { minDate, maxDate, excludeDates, includeDates, filterDate }: { minDate?, maxDate?, excludeDates?, includeDates?, filterDate?} = {}
) {
  return (
...
export function isDayExcluded(day, { excludeDates }: { excludeDates?} = {}) {
  return (
    (excludeDates &&
      excludeDates.some(excludeDate => isSameDay(day, excludeDate))) ||
    false
...
export function isOutOfBounds(day, { minDate, maxDate }: { minDate?, maxDate?} = {}) {
  return (
...
export function isTimeInDisabledRange(time, { minTime, maxTime }: { minTime?, maxTime?}) {
  if (!minTime || !maxTime) {
...
export function monthDisabledBefore(day, { minDate, includeDates }: { minDate?, includeDates?} = {}) {
...
export function monthDisabledAfter(day, { maxDate, includeDates }: { maxDate?, includeDates?} = {}) {
...
export function yearDisabledBefore(day, { minDate, includeDates }: { minDate?, includeDates?} = {}) {
...
export function yearDisabledAfter(day, { maxDate, includeDates }: { maxDate?, includeDates?} = {}) {
...
export function getEffectiveMinDate({ minDate, includeDates }: { minDate?, includeDates?}) {
...
export function getEffectiveMaxDate({ maxDate, includeDates }: { maxDate?, includeDates?}) {
