...
export default class DatePicker extends React.Component<ReactDatePickerProps, any> {
  ...
  calcInitialState = () => {
    ...
    const boundedPreSelection =
      minDate && defaultPreSelection && isBefore(defaultPreSelection, minDate)
        ? minDate
        : maxDate && defaultPreSelection && isAfter(defaultPreSelection, maxDate)
          ? maxDate
          : defaultPreSelection;
    return {
  ...
  handleChange = (...allArgs: [React.FocusEvent<HTMLInputElement>]) => {
    let event = allArgs[0];
    ...
    if (!isEqual(this.props.selected, changedDate) || this.props.allowSameDay) {
      if (changedDate !== null) {
        if (this.props.selected) {
          ...
          if (keepInput) { selected = newDate(changedDate) || new Date(); };
        ...
        if (
          this.props.inline && this.props.monthsShown &&
  ...
  setPreSelection = date => {
    ...
    if (date) {
      ...
      } else if (hasMinDate && this.props.minDate) {
        isValidDateSelection = isAfter(date, this.props.minDate);
      } else if (hasMaxDate && this.props.maxDate) {
  ...
  onInputKeyDown = event => {
    ...
    if (this.state.open) {
      ...
      if (!this.inputOk() && this.props.onInputError) {
  ...
  onDayKeyDown = event => {
    ...
    if (eventKey === "Enter") {
      ...
      this.setOpen(false);
      if (!this.inputOk()) {
        this.props.onInputError && this.props.onInputError({ code: 1, msg: INPUT_ERR_1 });
      }
    } else if (!this.props.disabledKeyboardNavigation && copy) {
      let newSelection;
      switch (eventKey) {
        case "ArrowLeft":
  ...
  renderDateInput = () => {
    ...
    const customInput: any = this.props.customInput || <input type="text" />;
